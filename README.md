...
第一次衰减（5e-4 → 2e-4）倍率 2.5×，第二次（2e-4 → 7e-5）倍率 2.9×。两次衰减幅度不相等，是基于一个简单的考量：**学习率越小，越需要留足余地**。5e-4 降到 2e-4 时，模型仍然有可观的更新动量，后续还有 90 轮可以用来调整。而 2e-4 降到 7e-5 后只剩 35 轮，如果降得太狠（如 5e-5），更新步长可能衰减到梯度信号无法有效驱动参数的程度，模型在最后阶段反而停滞。7e-5 是一个保守的上限——不会太小导致卡死，也不会太大导致末段剧烈震荡。
